%%%%%%%%%%%%%%%%%%% sfcn_snapPointOntoNearestTrajectory %%%%%%%%%%%%%%%%%%%
% Purpose:
%   Finds the pose on the interpolated trajectory that is nearest to the 
%   input vehicle position
% 
% INPUTS:
%   INPUT 1: 'ref_trajectory' is the complete vehicle_trajectory available 
%   in the database
%   INPUT 2: 'vehicle_cg_ahead' is the cg of the vehicle in ENU
% 
% OUTPUTS:
...
%
% Revision history:
% 2021/01/29: Function is updated based on 'fcn_Path_snapPointOntoNearestPath'
% 2021/03/23: 
% ======== to do list ============
% 1. Method to compute 'length_of_search_window' need to be updated
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sfcn_snapPointOntoNearestTrajectory(block)
...
if block.Dwork(1).Data
    % Happens only at the beginning of a new simulation
    %% Initialization
    % Check for repetitive station coordinates in the reference trajectory
    [~,unique_indices] = unique(ref_trajectory(:,fields_trajectory.station)); % find the indices with unique station coordinates
    ref_trajectory     = ref_trajectory(unique_indices,:); % trajectory with unique station coordinates
    ...
    start_index_for_local_search = start_index_for_local_search+first_path_point_index-1; % shift start_index depending on the nearest neighbour
    vehicle_trajectory = ref_trajectory(output_index,:); % Initialize the Output-1
    % update the output with estimated pose and station
    vehicle_trajectory(fields_trajectory.east)    = closest_path_point(1);
    vehicle_trajectory(fields_trajectory.north)   = closest_path_point(2);
