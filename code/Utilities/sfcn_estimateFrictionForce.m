%%%%%%%%%%%%%%%%%% S-Function sfcn_estimateFrictionForce %%%%%%%%%%%%%%%%%%
% Purpose:
...
% Revision History:
% 2020-06-09: Vectorization is implemented. It is faster than the
% commented output method. Removed for loops to write output.
% 2020-10-12: Fixed bug in vectorization and removed the commented out code
% 2020-11-10: Divide by zero cases are handled by replacing zero with eps.
% ======== to do list ============
% 1. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sfcn_estimateFrictionForce(block)
...
% p, q, Xi2, difference between Mz & Xi1 values corresponding to indices/tires with +ve p
pPositive   = p(~indicesNegativeP);
pPositive(pPositive == 0) = eps;    % To avoid divide by zero
qPositive   = q(~indicesNegativeP);
Xi2Positive = Xi2(~indicesNegativeP);
diff_Mz_XilPositive = diff_Mz_Xil(~indicesNegativeP);

% initialize output variable
output_temp = zeros(4,1);
...
if ~isempty(pPositive)
    ...
    % write frictional force to the output temporary variable
    output_temp(~indicesNegativeP) = frictionForcePositive;
